 ...
 * -------------------------------------------------------------
 */
cloud_void bErrorPrint(cloud_char* fmt, ...);
cloud_void displayBuffer(BufferReader* ptr_Buffer);
cloud_long getFileSize(cloud_char* fname);
cloud_int isNumber(const cloud_char* ns);
cloud_void startReader(cloud_char*, cloud_char*, cloud_char, cloud_int, cloud_int);
...
cloud_int mainReader(cloud_int argc, cloud_char** argv) {

	/* Create source input buffer */
	cloud_char* program = argv[0];
	cloud_char* input = argv[2];
	cloud_char mode = MODE_FIXED;
	cloud_int size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
cloud_void startReader(cloud_char* program, cloud_char* input, cloud_char mode, cloud_int size, cloud_int increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	cloud_int loadSize = 0;		/* the size of the file loaded in the buffer */
	cloud_char symbol;			/* symbol read from input file */
