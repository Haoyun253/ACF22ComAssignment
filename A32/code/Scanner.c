...
cloud_int line;								/* Current line number of the source code */
extern cloud_int errorNumber;				/* Defined in platy_st.c - run-time error number */

extern cloud_int stateType[];
extern cloud_char* keywordTable[];
extern PTR_ACCFUN finalStateTable[];
extern cloud_int transitionTable[][TABLE_COLUMNS];
...
Token tokenizer(cloud_void) {
	...
	cloud_char c;	/* input symbol */
	cloud_int state = 0;		/* initial state of the FSM */
	cloud_int lexStart;		/* start offset of a lexeme in the input char buffer (array) */
	cloud_int lexEnd;		/* end offset of a lexeme in the input char buffer (array)*/

	cloud_int lexLength;		/* token length */
	cloud_int i;				/* counter */
	cloud_char newc;			/* new char */
	...
	while (1) { /* endless loop broken by token returns it will generate a warning */
		...
		*/
		...
		default: // general case
			...
			lexemeBuffer = readerCreate((cloud_int)lexLength + 2, 0, MODE_FIXED);
			if (!lexemeBuffer) {
...
cloud_int nextState(cloud_int state, cloud_char c) {
	cloud_int col;
	cloud_int next;
	col = nextClass(c);
...
cloud_int nextClass(cloud_char c) {
	cloud_int val = -1;
	switch (c) {
	case CHRCOL2:
		val = 2;
...
Token funcIL(cloud_char lexeme[]) {
	...
	if (lexeme[0] != '\0' && strlen(lexeme) > NUM_LEN) {
		...
		if (tlong >= 0 && tlong <= SHRT_MAX) {
			...
			currentToken.attribute.intValue = (cloud_int)tlong;
		}
		else {
			currentToken = (*finalStateTable[ESNR])(lexeme);
		}
...
Token funcID(cloud_char lexeme[]) {
	...
	cloud_char lastch = lexeme[length - 1];
	cloud_int isID = CLOUD_FALSE;
	switch (lastch) {
	case MNIDPREFIX:
...
Token funcSL(cloud_char lexeme[]) {
	Token currentToken = { 0 };
	cloud_int i = 0, len = (cloud_int)strlen(lexeme);
...
cloud_void printToken(Token t) {
	...
	/* External variables */
	extern cloud_char* keywordTable[]; /* link to keyword table in */
	...
	case MNID_T:
		...
		break;
	case STR_T:
		printf("STR_T\t\t%d\t ", (cloud_int)t.attribute.codeType);
		printf("%s\n", readerGetContent(stringLiteralTable, (cloud_int)t.attribute.codeType));
		break;
	case LPR_T:
		printf("LPR_T\n");
		break;
...
Token funcFL(cloud_char lexeme[])
{
	...
	if (lexeme[strlen(lexeme) - 1] == '\0') {
		...
		if (treal >= 0.0 && treal <= FLT_MAX) {
			...
			currentToken.attribute.floatValue = (cloud_flout)treal;
		}
		else {
			currentToken = funcErr(lexeme);
		}
