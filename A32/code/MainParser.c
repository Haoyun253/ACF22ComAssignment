...
cloud_int errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern cloud_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern cloud_int line; /* source code line number - defined in scanner.c */
...
extern cloud_void startParser(cloud_void);
extern cloud_int startScanner(ReaderPointer sc_buf);

static cloud_void printParserError(cloud_char* fmt, ...);
static cloud_void displayParser(ReaderPointer ptrBuffer);
static cloud_long getParserFileSize(cloud_char* fname);
static cloud_void callGarbageCollector(cloud_void);
...
cloud_int mainParser(cloud_int argc, cloud_char** argv) {

	FILE* fi;       /* input file handle */
	cloud_int loadsize = 0; /*the size of the file loaded in the buffer */
...
cloud_void printParserError(cloud_char* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(cloud_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
cloud_long getParserFileSize(cloud_char* fname) {
	FILE* input;
	cloud_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
cloud_void displayParser(ReaderPointer ptrBuffer) {
...
cloud_void callGarbageCollector(cloud_void) {
